...
module.exports = require('enb/lib/build-flow').create()
    ...
    .useSourceListFilenames('waitForTargets', [])
    ...
    .builder(
        waitForSourcesAnd(
            function(/*sources*/) {
                ...
                return this.buildFile(null, path.join(this.node.resolvePath(this._source)));
            }
        )
    )
    .methods({
        ...
        readFile: function(parent, fullpath) {
            return vowFs
                ...
                .then(function(exists) {
                    if(!exists) {
                        var errStr = util.format('File %s, required by %s is not exists', fullpath, parent);
                        return vow.reject(new Error(errStr));
                    }
