...
module.exports = require('enb/lib/build-flow').create()
    ...
    .methods({
        ...
        processToken: function(parent, carrier, node, index) {
            ...
            if(excludes.indexOf(node.token) === -1) {
                ...
                if(matchFreeze.length > 0) {
                    return vow
                        .all(matchFreeze.map(function(x) {
                            return this.freezeFile(carrier, 0);
                        }, this));
                }
            }
         ...
         * @param {Array[vow.Promise -> freezePair, ...]} freezeMatches - Список промисов, возвращающих freezePair(#createFreezePair) по достижению успеха
